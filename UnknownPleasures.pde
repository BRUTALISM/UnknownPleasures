...
void draw() {
  ...
  // The height of each wave, as a percentage of pixel dimensions of the drawing (see dimension variable, below).
  ...
  // Calculate the vertical pixel spacing of waves based on the number of waves and drawing pixel dimensions
...
void drawWave(int row, int valueCount, float widthPixels, float heightPixels, float spread, float noiseCoefficient) {
  float horizontalSpacing = widthPixels / valueCount;
  ...
  for (int i = 1; i < valueCount; i++) {
    // The "envelope" is a common term denoting a function which is "wrapping" around another function. In this case,
    // the envelope is the gaussian ("bell") function, which is used to scale the values of Processing's noise()
    // function so that the middle part is more prominent.
